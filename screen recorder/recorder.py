...
class Recorder:
    ...
    def __init__(self, ffmpeg_manager, settings):
        ...
        self.audio = AudioManager(ffmpeg_manager.ffmpeg_path)
        self.process = None
    ...
    def build_command(self):
        ...
        if self.settings.get("microphone", False):
            mic_input = self.audio.get_mic_input()
            if self.audio.last_error:
                audio_error = self.audio.last_error
            audio_inputs += mic_input

        if self.settings.get("system_audio", False):
            sys_input = self.audio.get_system_audio_input()
            if self.audio.last_error:
                audio_error = self.audio.last_error
            audio_inputs += sys_input

        if audio_error:
            # Don't hand ffmpeg a command with a missing/broken audio
            # input - that produces a confusing failure instead of the
            # actual, actionable reason.
    ...
    def _extract_error(stderr_bytes):
        ...
        meaningful = meaningful or lines

        # "Error splitting the argument list: Option not found" is a
        # generic trailer ffmpeg prints AFTER the actually useful line
        # (e.g. "Unrecognized option 'foo'."). Prefer the specific line
        # if both are present, since the trailer alone isn't actionable.
        for ln in reversed(meaningful):
            if "error splitting the argument list" not in ln.lower():
                return ln
    ...
    def start(self):
        ...
        if cmd is None:
            # self.last_error was already set inside build_command()
            # (e.g. a required mic/system-audio device wasn't found).
            return
